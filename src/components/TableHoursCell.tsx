    ...
    <td className="border border-y-2">
      ...
      {startTime && endTime ? (
        extensionPresent ? (
          <button
            onClick={() => {
              setUserToggledExtensionOff(!userToggledExtensionOff);
            }}
            className={`mx-auto flex w-fit py-1 ${userToggledExtensionOff ? "bg-inherit" : "bg-red-500/10"} mb-1 cursor-default items-center justify-center rounded px-2 transition-all hover:-translate-y-0.5 hover:shadow-md hover:shadow-sky-900/40 active:translate-y-0 active:bg-sky-400/30`}
          >
            <svg
              className={`mx-2 size-4 ${userToggledExtensionOff ? "fill-gray-400" : "fill-red-400"} transition-all`}
              viewBox="0 0 16 16"
